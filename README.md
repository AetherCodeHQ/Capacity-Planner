...
> Capacity-Planner - Open source tool by AetherCodeHQ

`capacity-planning` `cli` `cloud` `devops` `golang` `open-source`

## What is it?

**Capacity Planner** is a devops tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Capacity-Planner.git
cd Capacity-Planner
...
```bash
# Basic usage
./Capacity-Planner --path ./target

# With options
./Capacity-Planner --path ./target --format json --output report.json

# Verbose mode
./Capacity-Planner --path ./target --verbose
```
...
### Basic scan
```bash
./Capacity-Planner --path ./my-project
```

### JSON report
```bash
./Capacity-Planner --path ./my-project --format json --output report.json
```
...
- name: Run Capacity Planner
  run: ./Capacity-Planner --path . --format json --output report.json
```
